...
@Composable
fun PositiveTrigger(positiveTrigger: List<Trigger>) {
    RoundedWhiteBox {
        Column(modifier = Modifier.fillMaxWidth()){
            Text("Positive Trigger", fontSize = 18.sp)
            Spacer(Modifier.padding(8.dp))
            positiveTrigger.forEachIndexed { index, trigger ->
                Spacer(modifier = Modifier.padding(4.dp))
                PositiveItem(image = when(index){
                    0->R.drawable.num1
                    1->R.drawable.num2
                    2->R.drawable.num3
